...
const PlacementAnalytics = () => {
  ...
  // Students for company dialog
  const companyDialogStudents = useMemo(() => {
    if (!companyDialog) return [];
    const studentById = new Map(students.map(s => [s._id, s]));
    const ids = companyStudentIdsMap.get(companyDialog) || new Set<string>();
    let list = [...ids].map(id => studentById.get(id)).filter(Boolean) as typeof students;
    ...
    return list;
  }, [companyDialog, students, companyStudentIdsMap, companySearchTerm]);
            ...
            <div className="space-y-4 mt-2">
              ...
              <div className="rounded-xl border overflow-hidden">
                <Table>
                  ...
                  <TableBody>
                    {companyDialogStudents.length === 0 ? (
                      <TableRow>
                      ...
                      companyDialogStudents.map((student, idx) => (
                        <TableRow key={student._id}>
                          <TableCell className="text-muted-foreground text-sm">{idx + 1}</TableCell>
                          <TableCell>
                            <div className="flex items-center gap-2">
                              <div className="w-7 h-7 rounded-full bg-success/10 text-success flex items-center justify-center text-xs font-bold shrink-0">
