...
const StudentStats = () => {
  ...
  // Map studentId â†’ their applications
                ...
                <TableBody>
                  {filteredStudents.length === 0 ? (
                    ...
                    filteredStudents.map(student => {
                      ...
                      const pending  = apps.filter(a => a.status === 'pending' || a.status === 'ongoing').length;
                      ...
                      return (
                        ...
                        >
                          ...
                          <TableCell className="text-sm font-mono">{student.registerNumber || 'â€”'}</TableCell>
                          <TableCell className="text-sm">{student.department || 'â€”'} {student.section ? `/ ${student.section}` : ''}</TableCell>
                          <TableCell>
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    <div className="flex items-center gap-2">
                      ...
                      <div>
                        ...
                        <p className="text-sm font-medium break-all">{selectedStudent.email || 'â€”'}</p>
                      </div>
                    </div>
                    <div className="flex items-center gap-2">
                      ...
                      <div>
                        ...
                        <p className="text-sm font-medium">{selectedStudent.phone || 'â€”'}</p>
                      </div>
                    </div>
                    <div>
                      <p className="text-xs text-muted-foreground">Register Number</p>
                      <p className="text-sm font-mono font-medium">{selectedStudent.registerNumber || 'â€”'}</p>
                    </div>
                    <div>
                      <p className="text-xs text-muted-foreground">Gender</p>
                      <p className="text-sm font-medium capitalize">{selectedStudent.gender || 'â€”'}</p>
                    </div>
                    <div>
                      <p className="text-xs text-muted-foreground">Department</p>
                      <p className="text-sm font-medium">{selectedStudent.department || 'â€”'}</p>
                    </div>
                    <div>
                      <p className="text-xs text-muted-foreground">Section</p>
                      <p className="text-sm font-medium">{selectedStudent.section || 'â€”'}</p>
                    </div>
                    <div>
                      ...
                      <p className={`text-sm font-bold ${(selectedStudent.gpa || 0) >= 8 ? 'text-success' : (selectedStudent.gpa || 0) >= 6 ? 'text-warning' : 'text-destructive'}`}>
                        {selectedStudent.gpa?.toFixed(2) ?? 'â€”'}
                      </p>
                    </div>
                    <div>
                        ...
                        {selectedApps
                          ...
                          .map(app => (
                            <TableRow key={app._id}>
                              <TableCell className="font-medium">{app.companyId?.name || 'â€”'}</TableCell>
                              <TableCell className="text-sm text-muted-foreground">{app.companyId?.location || 'â€”'}</TableCell>
                              <TableCell className="text-sm">
                                {app.companyId?.package ? `â‚¹${app.companyId.package} LPA` : 'â€”'}
                              </TableCell>
                              <TableCell className="text-sm text-muted-foreground">
                                {app.appliedDate ? new Date(app.appliedDate).toLocaleDateString() : 'â€”'}
                              </TableCell>
